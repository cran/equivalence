...
"equiv.boot" <-
  function(x, y, alpha=0.05, b0.ii=0.25, b1.ii=0.25, reps=100,
    ...
    c.b0.l <- quantile(plot.boot$t[,7], probs=alpha)
    c.b0.u <- quantile(plot.boot$t[,7], probs=1-alpha)
    ...
    i.b0.u <- ifelse(b0.ii.absolute,
                     mean(x, na.rm = TRUE) + b0.ii,
                     mean(x, na.rm = TRUE) * (1 + b0.ii))
    c.b1.l <- quantile(plot.boot$t[,8], probs=alpha)
    c.b1.u <- quantile(plot.boot$t[,8], probs=1-alpha)
    i.b1.l <- 1 * (1 - b1.ii)
    i.b1.u <- 1 * (1 + b1.ii)
